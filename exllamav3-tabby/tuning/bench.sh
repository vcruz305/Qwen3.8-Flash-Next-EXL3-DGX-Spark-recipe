...
export EXL3_INT8_GEMV=${EXL3_INT8_GEMV:-0} EXL3_MOE_COOP_WIDE=${EXL3_MOE_COOP_WIDE:-1} EXL3_GR_INT8=${EXL3_GR_INT8:-1} EXL3_MTP_HEAD_N=${EXL3_MTP_HEAD_N:-65536} EXL3_NGRAM_STREAM=${EXL3_NGRAM_STREAM:-0}
cd "$HOME/exllamav3"
"$TUNING_DIR/../drop-model-cache.sh" >/dev/null
...
/usr/bin/time -f "WALL %e s  MAXRSS %M KB" \
  taskset -c 5-9,15-19 python examples/chat.py -m "$HOME/models/Qwen3.8-Flash-Next-EXL3" -mode qwen35 -cs 32768 -tps -basic -lm -no_think \
    -topk 1 -maxr 400 -prompt "$PROMPT" "$@" > "$LOG" 2>&1
echo "=== $TAG  flags: $* ==="
