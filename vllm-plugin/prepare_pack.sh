...
#
# WARNING: this rewrites config.json and model.safetensors.index.json IN PLACE (backups:
# *.native). exllamav3/TabbyAPI cannot load the rewritten pack. If you also run the primary
# exllamav3-tabby path, point it at a native view afterwards:
#   bash exllamav3-tabby/tools/make_native_view.sh <pack> <pack>-native
set -euo pipefail
...
if ! grep -q '"sharded"' "$TOOLS/qwen_pack_scan.py"; then
  ...
  echo "With an older plugin, vllm-plugin/rename_unsharded_ngram.py is the workaround for unsharded packs." >&2
  exit 1
fi
